...
@description('Optional. The name of the diagnostic setting, if deployed.')
param diagnosticSettingsNameaudit string = '${name}-auditdiagnosticSettings'
...
var diagnosticsLogsSpecifiedaudit = [for category in filter(diagnosticLogCategoriesToEnableAudit, item => item != 'Audit'): {
  category: category
  enabled: true
  retentionPolicy: {
    enabled: true
...
var diagnosticsLogsaudit = contains(diagnosticLogCategoriesToEnableAudit, 'Audit') ? [
  {
    categoryGroup: 'Audit'
    enabled: true
    retentionPolicy: {
...
resource configurationStore_diagnosticSettingsaudit 'Microsoft.Insights/diagnosticsettings@2021-05-01-preview' = if ((!empty(diagnosticStorageAccountId)) || (!empty(diagnosticWorkspaceIdaudit)) || (!empty(diagnosticEventHubAuthorizationRuleId)) || (!empty(diagnosticEventHubName))) {
...
module configurationStore_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
