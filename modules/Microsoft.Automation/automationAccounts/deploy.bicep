...
resource automationAccount_diagnosticSettingsAudit 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if ((!empty(diagnosticStorageAccountId)) || (!empty(diagnosticWorkspaceIdAudit)) || (!empty(diagnosticEventHubAuthorizationRuleId)) || (!empty(diagnosticEventHubName))) {
...
module automationAccount_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
