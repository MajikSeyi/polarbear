...
@description('Optional. The name of the diagnostic setting, if deployed. If left empty, it defaults to "<resourceName>-diagnosticSettings".')
param diagnosticSettingsNameaudit string = ''
...
resource topic_diagnosticSettingsaudit 'Microsoft.Insights/diagnosticsettings@2021-05-01-preview' = if ((!empty(diagnosticStorageAccountId)) || (!empty(diagnosticWorkspaceIdaudit)) || (!empty(diagnosticEventHubAuthorizationRuleId)) || (!empty(diagnosticEventHubName))) {
  name: !empty(diagnosticSettingsNameaudit) ? diagnosticSettingsNameaudit : '${name}-diagnosticSettingsaudit'
  properties: {
    storageAccountId: !empty(diagnosticStorageAccountId) ? diagnosticStorageAccountId : null
...
module topic_privateEndpoints '../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
...
output princpleid string = topic.identity.principalId
