...
module slot_appsettingsfunction 'config-appsettings/deployfunction.bicep' = if (!empty(appSettingsKeyValuePairsfunction)) {
  name: '${uniqueString(deployment().name, location)}-Slot-${name}-Config-AppSettings'
  params: {
    slotName: slot.name
    appName: app.name
...
module slot_privateEndpoints '../../../Networking/privateEndpoints/Privateendpoint.bicep' = [for (privateEndpoint, index) in privateEndpoints: {
