...
param maintenanceassign string = ''
...
resource maintenanceassignment 'Microsoft.Maintenance/configurationAssignments@2023-04-01' = if (!empty(maintenanceassign)) {
  scope: maintenanceConfiguration
  name: '${maintenanceConfiguration.name}-assign'
  properties: {
      maintenanceConfigurationId: maintenanceConfiguration.id 
      resourceId: maintenanceassign
  }
}
...
@description('The resource ID of the Maintenance Configuration.')
output namespace string = maintenanceConfiguration.properties.namespace
